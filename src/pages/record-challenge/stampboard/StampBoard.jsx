...
const StampBoard = () => {
    ...
    return (
            ...
            <Styles.StampWrapper>
                ...
                {rows.map((row, rowIndex) => (
                    <Styles.Row key={rowIndex}>
                        {row.map((item, index) => (
                            <Styles.Item key={index} rowLength={row.length} onClick={() => clickHandler(item)}>
                                <Stamp data = {item} />
                            </Styles.Item>    
                        ))}
                    </Styles.Row>
        ...
        <Styles.SubText style = {{fontSize: 'var(--font-size-xs)'}}>
            ...
            스탬프가 정상 인증되지 않는경우 고객센터로 문의하세요<br/></Styles.SubText>
        </Styles.CautionWrapper>
        </>
    );
}
